...
class Home(ft.Container):
    ...
    def __init__(self, player: models.Player, page: ft.Page):
        ...
        super().__init__()
        self.expand = True
        self.tabContent = ft.Container(
            content=ft.Text("Index did not change!"), expand=True
        )
                    ...
                    destinations=[
                        ft.NavigationRailDestination(
                            ...
                            selected_icon_content=ft.Icon(
                                ft.icons.HOME_ROUNDED, size=35
                            ),
                            label="Home",
                        ),
                        ft.NavigationRailDestination(
                            icon_content=ft.Icon(ft.icons.SEARCH_OUTLINED, size=35),
                            selected_icon_content=ft.Icon(ft.icons.SEARCH, size=35),
                            label="Search",
                        ),
                        ft.NavigationRailDestination(
                            icon_content=ft.Icon(
                                ft.icons.FIND_IN_PAGE_OUTLINED, size=35
                            ),
                            selected_icon_content=ft.Icon(
                                ft.icons.FIND_IN_PAGE_ROUNDED, size=35
                            ),
                            label="Browse",
                        ),
                        ft.NavigationRailDestination(
        ...
        if e.key == "Enter" and self.content.controls[0].content.selected_index == 1:
            ...
            if (query != "" and query != None) and not self.isBusy:
                ...
                searchResultsWidget = SearchResults(
                    results=response, player=self.player
                )
                self.resultsPage = ft.Column(
                    controls=[Search_bar_widget(query=query), searchResultsWidget],
                    expand=True,
                )
                self.updateTheUI(1)
            self.isBusy = False
        ...
        match selectedItem:
            case 0:
                if not self.suggestions:
                    ...
                    self.suggestions = ft.Column(
                        controls=[SuggestionsWidget(suggestions, player=self.player)],
                        expand=True,
                    )
                    self.updateTheUI(selectedItem)
            case 1:
                if not self.resultsPage:
                    self.resultsPage = ft.Column(
                        controls=[Search_bar_widget()], expand=True
                    )
                    self.updateTheUI(1)
            case 2:
...
class SuggestionsWidget(ft.ListView):
    ...
    def __init__(
        self, results: yt_models.GetSuggestionsResponse, player: models.Player
    ):
        self.player = player
...
class SearchResults(ft.ListView):
    def __init__(self, results: yt_models.SearchResponse, player: models.Player):
        ...
        songsWidgets = []
        [
            artistsWidgets.append(SquareArtistWidget(artist=artist))
            for artist in results.artists
        ]
        [
            albumsWidgets.append(SquareAlbumWidget(album=album))
            for album in results.albums
        ]
        self.controls.append(albumsWidgets)
        [
            self.controls.append(
                SongWidget(song=song, player=player, songList=results.songs)
            )
